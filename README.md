...
│                                                               │
│  ┌─────────────────────────────────────────────────────┐    │
│  │               Vue.js 2.x SPA (Hash Router)          │    │
│  │                                                       │    │
│  │  ┌──────────┐  ┌──────────┐  ┌──────────────────┐  │    │
│  │  │ Vue Router│  │  Vuex    │  │  国际化 (11语言)  │  │    │
│  │  │ 94+ routes│  │ 7+ states│  │  vue-i18n        │  │    │
│  │  └──────────┘  └──────────┘  └──────────────────┘  │    │
│  │                                                       │    │
│  │  ┌──────────────────────────────────────────────┐    │    │
│  │  │              业务功能层 (25模块)              │    │    │
...
│  │  │  UI 层   │  │  地图层   │  │    可视化层       │  │    │
│  │  │ layui    │  │ 百度地图  │  │    ECharts        │  │    │
│  │  │ zTree    │  │ OpenLayers│  │    bar/line/pie   │  │    │
│  │  └──────────┘  └──────────┘  └──────────────────┘  │    │
│  │                                                       │    │
│  │  ┌──────────┐  ┌──────────┐  ┌──────────────────┐  │    │
│  │  │ 工具库   │  │  视频层   │  │    数据层         │  │    │
│  │  │ CryptoJS │  │  hls.js  │  │  Axios + Socket   │  │    │
│  │  │ xlsx     │  │          │  │  localStorage     │  │    │
...
│                                                               │
│  /              → index.html (SPA入口)                       │
│  /static/*      → Webpack 构建产物 (manifest+vendor+278chunk)│
│  /static/layui/ → layui 框架静态文件                          │
│  /api/*         → proxy_pass → 后端 API 服务                  │
└──────────────────────────┬──────────────────────────────────┘
                           │
                           ▼
┌─────────────────────────────────────────────────────────────┐
...
│                                                               │
│  ┌──────────┐  ┌──────────┐  ┌──────────┐  ┌──────────┐   │
│  │ 认证服务  │  │ 设备管理  │  │ GPS 上报  │  │ 支付系统  │   │
│  │ /api/auth │  │/api/devices│ │ /api/gps │  │/api/pay  │   │
│  └──────────┘  └──────────┘  └──────────┘  └──────────┘   │
│  ┌──────────┐  ┌──────────┐  ┌──────────┐  ┌──────────┐   │
│  │ 围栏服务  │  │ 告警服务  │  │ 轨迹查询  │  │ OTA升级  │   │
│  │/api/geof  │  │/api/alerts│  │/api/tracks│  │  ota*    │   │
│  └──────────┘  └──────────┘  └──────────┘  └──────────┘   │
│                                                               │
│                         ┌──────────┐                         │
│                         │  数据库   │                         │
...
| 模式 | 代表页面 | 结构 | 地图 |
|------|---------|------|------|
| **检索+地图** | 监控平台、视频直播 | GlobalSidebar + 左侧面板(300px) + 右侧地图 | ✅ |
| **三级导航** | 我的设备、数据统计、系统设置 | GlobalSidebar + SubSidebar(180px) + 左侧面板(可选) + 工作区 | ❌ |
| **三栏GIS** | 电子围栏 | GlobalSidebar + 左侧面板(300px) + 中间列表(320px) + 右侧地图 | ✅ |
| **全屏浮动** | 历史轨迹 | GlobalSidebar + 全屏地图 + FloatingPanelGroup 叠加 | ✅ |

### 左侧菜单体系

```
GlobalSidebar (110px, dark)
├── 主功能菜单组 (8项, top-aligned)
│   ├── 监控平台    (monitor)
│   ├── 我的设备    (device)    → SubSidebar (180px, 11项)
│   ├── 数据统计    (bar-chart) → SubSidebar (180px, 4项)
│   ├── 电子围栏    (vector-polygon)
│   ├── 远程指令    (command-terminal)
│   ├── 视频直播    (video-live)
│   ├── 报警列表    (bell-alert)
│   └── 历史轨迹    (history-route)
│
└── 底部系统工具组 (3项, bottom-aligned)
    ├── 客户列表    (users)     → 展开可伸缩面板 (320px, resizable)
    ├── 切换语言    (global)
    └── 车在这儿Logo (car-location)
```
...
GPS 设备 → 后端 API ←→ 数据库
                │
                ├── REST API (/api/*) ──→ Axios ──→ Vuex Store ──→ 组件渲染
                │
                └── WebSocket ──→ Socket.io ──→ 实时更新
                                                    │
                        ┌───────────────────────────┤
                        ▼                           ▼
              device:position                 alert:new
              (位置实时推送)                  (告警实时推送)
                        │                           │
                        ▼                           ▼
              deviceListMap[terminalID]       alarmSound 通知
              .update(data)
```
...
被复用页面：
  监控平台 │ 我的设备(全11页) │ 数据统计(全4页)
  电子围栏 │ 远程指令 │ 视频直播 │ 历史轨迹(悬浮态)
```
...
│     业务层 (25个功能模块)     │  ← 自研商业逻辑
├──────────────────────────────┤
│    框架层 (Vue 2 + Router)    │  ← MVVM + SPA路由
├──────────────────────────────┤
│     状态层 (Vuex)             │  ← 全局状态管理
├──────────────────────────────┤
│     UI 层 (layui + zTree)     │  ← 表格/表单/树/弹窗
├──────────────────────────────┤
│    地图层 (百度地图+OpenLayers)│  ← 双引擎GIS
├──────────────────────────────┤
│    可视化层 (ECharts)         │  ← 统计图表
├──────────────────────────────┤
│    通信层 (Axios + Socket.io)  │  ← HTTP + WebSocket
├──────────────────────────────┤
│    构建层 (Webpack 3.x)       │  ← 278 chunk懒加载
├──────────────────────────────┤
│    部署层 (Nginx 1.10.2)      │  ← 反向代理+静态资源
└──────────────────────────────┘
```
...
| 决策 | 选择 | 原因 |
|------|------|------|
| 前端框架 | Vue 2.x (非 React/Angular) | 国产生态、layui 兼容 |
| 地图方案 | 百度地图 (默认) + OpenLayers | 国内 GPS 场景，百度地图数据更全 |
| UI 框架 | layui (非 Element/Ant Design) | 2017-2018 年主流选择，经典稳定 |
| 构建工具 | Webpack 3.x (非 Vite) | 部署于 2025-03，当时 Webpack 主流 |
| 路由模式 | Hash 路由 (#/) | Nginx 无需特殊配置 |
| 国际化 | 11 种语言 | 面向东南亚/一带一路市场 |
| 状态管理 | Vuex | Vue 2 官方方案 |
| 加密方案 | CryptoJS AES (前端) | 快速实现，密钥硬编码 |
